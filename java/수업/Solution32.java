	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1; i <= tc; i++) {
			...
			for (int j = 0; j < player_n; j++) {
				player[j][0] = (sc.nextInt() - 1);
				player[j][1] = (sc.nextInt() - 1);
			...
			for (int j = 0; j < spot_n; j++) {
				int spot_x = (sc.nextInt() - 1);
				int spot_y = (sc.nextInt() - 1);
				map[spot_x][spot_y] = 0;
			}
			showmap(map);
			...
			for (int j = 0; j < player_n; j++) {
				...
				for (int k = 0; k < player[j][2]; k++) {
					if (isIn(move(start_r, start_c, map)[0], move(start_r, start_c, map)[1], map_r, map_c)) {
						System.out.println("#1r:" + start_r + "c:" + start_c);
						start_r = move(start_r, start_c, map)[0];
						start_c = move(start_r, start_c, map)[1];
